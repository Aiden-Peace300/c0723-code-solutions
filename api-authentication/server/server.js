...
app.post('/api/auth/sign-in', async (req, res, next) => {
  try {
    ...
    if (!username || !password) {
      ...
      // Create a new signed token using jwt.sign() and your TOKEN_SECRET
      const token = jwt.sign(payload, process.env.TOKEN_SECRET);

      // Send a 200 response containing the payload and token
      res.status(200).json({
        token,
        payload,
      });
    }
    /* DONE Query the database to find the "userId" and "hashedPassword" for the "username".
     * If no user is found,
